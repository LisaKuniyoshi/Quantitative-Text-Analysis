...
* [quant\_text\_analysis.mnlr.statsmodels\_fork](#quant_text_analysis.mnlr.statsmodels_fork)
  ...
  * [pairwise\_ame\_mnlogit](#quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_mnlogit)
...
- `df_obs` _pd.DataFrame_ - ``doc_id`` ``code`` ``year`` ``method`` 列を持つ長形式データ。
- `base_method` _str, optional_ - ``method`` の基準水準。既定は ``"qual"`` 。
...
- `df_pred` _pd.DataFrame_ - 予測対象データ。``year_centered`` ``method`` などの列を含む。
...
<a id="quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_mnlogit"></a>

#### pairwise\_ame\_mnlogit

```python
def pairwise_ame_mnlogit(rob: MultinomialResults | MultinomialResultsWrapper,
                         factor: str = "method",
                         base: str = "qual",
                         at: str = "overall",
                         alpha: float = 0.05,
