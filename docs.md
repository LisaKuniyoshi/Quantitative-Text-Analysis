...
* [quant\_text\_analysis.mnlr.statsmodels\_fork](#quant_text_analysis.mnlr.statsmodels_fork)
  * [t\_test\_pairwise\_mnlogit](#quant_text_analysis.mnlr.statsmodels_fork.t_test_pairwise_mnlogit)
  * [pairwise\_ame\_categorical](#quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_categorical)
  * [pairwise\_ame\_multihot](#quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_multihot)
...
- `groups` _list[list[str]] | None_ - 文書が属するグループラベル。
...
- `per_doc_tokens` _Sequence[Sequence[str]]_ - 文書ごとのトークン列。
- `years` _pandas.Series_ - 各文書の出版年。
- `methods` _pandas.Series_ - 各文書の研究手法ラベル集合（list[str]）。
...
- `df_obs` _pd.DataFrame_ - ``doc_id`` ``code`` ``year`` および
...
```python
def predict_probabilities(res: MultinomialResultsWrapper,
...
- `res` _MultinomialResultsWrapper_ - `fit_mnlogit` が返す推定結果の一つ。
- `df_pred` _pd.DataFrame_ - 予測対象データ。``year_centered`` と
  `method_*` ダミー列など、推定時と同じ特徴量を含む。
- `cats` _List[str]_ - 予測確率の列順に対応するコード名のリスト。
...
<a id="quant_text_analysis.mnlr.statsmodels_fork"></a>

# quant\_text\_analysis.mnlr.statsmodels\_fork

<a id="quant_text_analysis.mnlr.statsmodels_fork.t_test_pairwise_mnlogit"></a>

#### t\_test\_pairwise\_mnlogit

```python
def t_test_pairwise_mnlogit(res: MultinomialResults
                            | MultinomialResultsWrapper,
                            term_name: str,
                            alpha: float = 0.05,
                            mt_method: str = "holm-sidak") -> pd.DataFrame
```
...
- `term_name` _str_ - 係数名の接頭辞（例: ``"C(method, Treatment('qual'))"``）。
- `alpha` _float, optional_ - 多重比較補正後に用いる有意水準。既定は ``0.05`` 。
- `method` _str, optional_ - 多重比較補正の方式。``holm-sidak`` や ``bonferroni`` など。
...
<a id="quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_categorical"></a>

#### pairwise\_ame\_categorical

```python
def pairwise_ame_categorical(rob: MultinomialResults
                             | MultinomialResultsWrapper,
                             factor: str = "method",
                             base: str = "qual",
                             at: str = "overall",
...
- `rob` _MultinomialResults | MultinomialResultsWrapper_ - ロバスト共分散（任意）を
  含む MNLogit 推定結果。
- `factor` _str, optional_ - 対象となるカテゴリ説明変数名。既定は ``"method"`` 。
- `base` _str, optional_ - Treatment 符号化における基準水準。既定は ``"qual"`` 。
- `at` _str, optional_ - `get_margeff` に渡す ``at`` 引数。既定は ``"overall"`` 。
- `alpha` _float, optional_ - 多重比較補正後に用いる有意水準。既定は ``0.05`` 。
- `mt_method` _str, optional_ - 多重比較補正の方式。既定は ``"holm-sidak"`` 。
...
<a id="quant_text_analysis.mnlr.statsmodels_fork.pairwise_ame_multihot"></a>

#### pairwise\_ame\_multihot

```python
def pairwise_ame_multihot(rob: MultinomialResults | MultinomialResultsWrapper,
